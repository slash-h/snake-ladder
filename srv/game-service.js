...
module.export = class GameService extends cds.ApplicationService {
  async init() {

    const { Players, BoradSquares } = this.entities;
    ...
    this.on("rollDice", "Players", async (req) => {
      ...
      // Validate that the newPosition doesn't go beyond 100 (the BoradSquares boundry)
      if (newPosition > 100) {
      ...
      const square = await SELECT.one.from(BoradSquares).where({ square: newPosition });
